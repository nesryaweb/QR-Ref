...
export function calculateGradeTotals(grade) {
  ...
  const subjectAverages = subjects
    .map((subject) =>
      calculateAverage(subject.firstSemester, subject.secondSemester),
    )
    .filter((average) => average !== "");

  const overallAverage =
    subjectAverages.length > 0
      ? subjectAverages.reduce((total, average) => total + average, 0) /
  ...
  return {
    ...
    firstSemesterAverage:
      firstSemesterAverage === "" ? "" : round(firstSemesterAverage),

    secondSemesterAverage:
      secondSemesterAverage === "" ? "" : round(secondSemesterAverage),

    overallAverage: overallAverage === "" ? "" : round(overallAverage),
  };
}
