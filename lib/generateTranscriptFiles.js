...
export async function generateTranscriptFiles(reference) {
  ...
  try {
    ...
    await page.setExtraHTTPHeaders({
      "x-transcript-secret":
        process.env.TRANSCRIPT_INTERNAL_SECRET || "",
    });
    ...
    await page.evaluate(() => {
      const transcript =
        document.getElementById("transcript-document");

      if (!transcript) {
        throw new Error(
          "Transcript document not found.",
        );
      }
      ...
      transcript.style.boxSizing =
        "border-box";

      transcript.style.transformOrigin =
        "top left";
